...
\section{Entropy, Cross-Entropy, KL
Divergence}\label{entropy-cross-entropy-kl-divergence}

There are a number of ways to think about/get to the Shannon entropy
(\(H\)) (thankfully not \(S\)...) One simple way is to say what we want
...
\textbf{cross-entropy} (because that sounds pretty cyberpunk to me. I
...
This metric is called the \textbf{Kullback--Leibler divergence} (because
...
This makes describing the \textbf{mutual information} of random
variables X and Y in terms of a KL divergence fairly intuitive. Just
running off the name, if X and Y were independent, you'd expect no
mutual information between them. In such a case, \(P(X,Y) = P(X)P(Y)\),
and \() KL\left(P(X,Y)\mid\mid P(X)P(Y)\right) = 0\). And in fact, one
way of writing the mutual information of random variables X and Y is
exactly
...
Also worth knowing the fancy terminology: - the \textbf{\emph{a priori}
probability} or just the \textbf{prior} is what we thought would be the
...
\(A\) and so \(P(A)\) would be an \textbf{\emph{a priori} distribution}
(or again, just the prior). - the \textbf{\emph{a posteriori}
probability} or just the \textbf{posterior} is what we think the
...
\textbf{\emph{a posteriori} distribution}, (or again, just the
posterior).
...
up with \(P(B)\)! ('!' used to denote excitement, not factorialization.)
...
\item
  ...
  \(f(x,y) = m\) (i.e., the \textbf{level set} of \(f\) corresponding to
...
the \textbf{Lagrangian} of \(f(x)\) (because dude needs more things
...
\section{Explaining the convenience, and a more generalizable intuition.}\label{explaining-the-convenience}
...
pretty much verbatim -- \textbf{if \(\nabla f\) weren't perfectly
perpendicular to \(\bigcap _i G_i\), we'd be wasting the component of
the gradient going along \(\bigcap _i G_i\), \(\nabla f ^{\parallel}\).
So we'd just step along the contour and improve our result.} 
(Excessive bolding used to emphasize the fact that this is the more
...
writing this in \(LaTeX\) and not Markdown.} Well, since we've migrated to LaTeX, I think we owe ourselves a box!
...
equality-constraints-only version. That is, the dual problem is simply a
...
\item
  ...
  \(g_i > 0\) (our optimal-point finder never hit a wall -\/- there's an
...
at first. But it's really not that bad! The main intuition is that: 1.
either the inequality constraint is lax and doesn't actually do any
``constraining"; or 2. the inequality constraint is actually stopping us,
