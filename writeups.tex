...
amount of energy in the system \(E_{{T}}\),
...
the closer \(\Delta E\) is to zero, and when \(\Delta E = 0\), independence holds exactly
(This is because 
there would always be an infinite number of \(\Delta E\)'s to choose from, no matter
how many had been taken up via assignment to previous particles \textemdash{} a weird thought,
...
is at \(E = x\Delta E\). That means unlike \(x\), as \(\Delta E \rightarrow 0\), the values
\(E\) can take on cover more and more granular, to the point where \(E\) becomes arbitrarily
...
\begin{enumerate}
  ...
  \item
    ...
    \(\Delta E(\nu) = nh\nu\), with \(n \in \mathcal{R}\) 
    and \(h\) is \textbf{Planck's constant}\index{Planck's constant},
    \(h \approx \SI{6.626}{J.s}\).
    In this case, we'll have to accept that our probability function, having been derived
    based on the assumption of continuous \(E\) (as described above), is only an approximation.
\end{enumerate}
...
More specifically, again remembering our mapping \(x\Delta E \mapsto E\),
we'll see the following expression again:

\[\left<E\right> = \left<x\Delta E\right> = \frac{\Delta E}{e^{a\Delta E} - 1} \]

and at that point, we'll be trying to derive \(\left<E\right>\) given \(a\) (and \(\Delta E\))
rather than derive \(a\) given \(\left<E\right>\).
\\
...
% Mention the differences/different assumptions between 
% Boltzmann vs. Bose-Einstein vs Fermi-Dirac


% where \(C\) is a normalization constant.

% Now let's define the function in terms of energy so we don't have to index by \(i\)
% (which kind of becomes more confusing than helpful the less we talk about \(\Delta E\)'s):

% \[Pr[\text{particle in system has energy }E] = p_{+}(E) = C\times e^{\frac{E}{E_0}}\]

% And you know, I think it's a \emph{tad} unlikely that a particle is more likely to be in high
% energy levels than low ones (especially considering our calculations have shown a decrease in
% probability as energy level increased), 
% so let's first guess that the exponent is negative \textemdash{} if we're somehow wrong,
% \(E_0\) will be negative:

% \[Pr[\text{particle in system has energy }E] = p(E) = A\times e^{-\frac{E}{E_0}}\]

% OK then, how do we find \(E_0\)?
% (Spoiler alert: the gut feeling of \(\left<E\right>\) is correct in this case.)
% \\
% Well, we want \(p(E)\) to fit our data as well as possible.


% Remember that all of this presumed a particular total amount of energy \(E_T\). The shape
% of the distribution should change as \(E_T\) changes. How does it change?


% TODO: Talk about distribution shape,
% differences between Boltzmann vs Bose vs Fermi distributions


% \(Pr[i+j]\) is proportional to the probability of any specific energy pair occurring,
% e.g., \(i, j\). So then \(Pr[i+j] = A\times Pr[i,j]\)
% ,
% and we have \(\Delta E \rightarrow 0\). 



% We've previously assumed that all microstates have equal probability of occurring.
% This comes in handy here. 
% As \(N \righarrow \infty\), and because we're dealing with
% indistinguishable particles, \emph{we can map every energy pair
% above to an \textbf{identical} microstate} \textemdash{} if the energy pair is \((a,b)\),
