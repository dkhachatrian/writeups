...
The expression for our pseudo-residuals seem to come out of nowhere, but let's
...
% \section{Mutual information and conditional entropy.}\label{mutual-information-and-conditional-entropy}

% This makes describing the \textbf{mutual information between two random
% variables \(X\) and \(Y\)} (or \textbf{information gain between two random variables})in terms of a KL divergence fairly intuitive. Just
% running off the name, if X and Y were independent, you'd expect no
% mutual information between them --- 
% observing one variable wouldn't tell you anything about the other variable. 
% In such a case, \(P(X,Y) = P(X)P(Y)\), and 
% \( KL\left(P(X,Y)\mid\mid P(X)P(Y)\right) = 0\). 
% And in fact, one way of writing the mutual information 
% between random variables X and Y is exactly
...
% which I think is pretty neat.
% \\
% In fact, let's play this game of ``critical thinking'' a bit more. 
% Just a second ago, we were talking about whether Y gives information about X
% (or vice-versa -- let's stick with Y for now). If Y \emph{does} give information
% about X and we've observed Y already, we'll be less surprised by the value of X
% we do end up seeing. Hmm, surprise... Sounds like an extension to entropy is 
% right in front of us! Let's go for it.

% We want the \textbf{conditional entropy} of a random variable Y with respect to X 
% to have the relationship where

% \[ H(X) + H(Y|X)  \]

% \[
%   \begin{split}
%       H(Y \mid X) &= E_{x \sim X}\left[H(Y)\right] \\
%                   &= \sum_{x \in X} P(x) \times H(Y|X = x)
%   \end{split}
% \]

% Let's expand \(H(Y|X = x)\):

% \[
%   H(Y| X = x) = \sum_{y \in Y} p(y|x) \times \log\left(\frac{1}{p(y|x)}\right)
% \]

% Now we stick it back up above and play with it a bit:
